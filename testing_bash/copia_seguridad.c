...
int main(__attribute__((unused)) int ac, char **av)
{
        ...
        do {
                ...
                vars.buffer = NULL;
                len_buffer = 0;
                parse_line(vars.buffer, len_buffer, command_counter, av);
...
void parse_line(vars_t vars, size_t len_buffer, int command_counter, char **av)
{
        ...
        {
                vars.array_tokens = token_interface(vars.buffer, delimiter, to>
                if (vars.array_tokens[0] == NULL
                {
                        single_free(2, vars.array_tokens, vars.buffer);
                        return;
                }
...
char **token_interface(vars_t vars, const char *delimiter, int token_count)
{
        token_count = count_token(vars.buffer, delimiter);
        if (token_count == -1)
        {
                free(vars.buffer);
                return (NULL);
        }
        vars.array_tokens = tokenize(token_count, vars.buffer, delimiter);
